...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeNorthernAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNorthernAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeNorthernAmount = strAnonymizeNorthernAmount.remove(strAnonymizeNorthernAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNorthernAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeNorthernAmount * COIN) nMaxToAnonymize = nAnonymizeNorthernAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeNorthernAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeNorthernAmount));
        strAnonymizeNorthernAmount = strAnonymizeNorthernAmount.remove(strAnonymizeNorthernAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNorthernAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
